...
//Time: nlogn
//Space: n
class CompareMapEntry implements Comparator<Map.Entry<String, Integer>> {
    @Override
...
class Solution {
    public List<String> topKFrequent(String[] words, int k) {
        ...
        PriorityQueue<Map.Entry<String, Integer>> sortedWords = new PriorityQueue<>(k, new CompareMapEntry());
        sortedWords.addAll(frequencyMap.entrySet());
        // Extract the top k frequent words
        List<String> topKWords = new ArrayList<>();
        for (int i = 0; i < k && sortedWords.size() > 0; i++) {
